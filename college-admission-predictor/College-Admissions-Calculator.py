...
class CollegeAdmissionPredictor:
    ...
    def preprocess_data(self, data):
        ...
        if HAS_IMBLEARN:
            ...
            print("SMOTE not available. Proceeding without class balancing.")
    ...
    def train_models(self, X_train, y_train):
        ...
        models = {
            ...
            'XGBoost': xgb.XGBClassifier(random_state=42, eval_metric='logloss') if HAS_XGBOOST else None,
            'SVM': SVC(random_state=42, probability=True)
        }
        # Remove None models (if XGBoost is not available)
...
def get_user_input(predictor):
    """Prompt user for student features and return as a list in correct order"""
    print("\nEnter student details for admission prediction:")
    gpa = float(input("GPA (0.0 - 4.0): "))
    sat_score = float(input("SAT Score (400 - 1600): "))
    act_score = float(input("ACT Score (1 - 36): "))
    extracurricular = int(input("Extracurricular Activities (0 - 10): "))
    leadership = int(input("Leadership Positions (0 - 3): "))
    volunteer_hours = float(input("Volunteer Hours (0 - 500): "))
    work_experience = float(input("Work Experience (months, 0 - 48): "))
    family_income = float(input("Family Income (20000 - 300000): "))
    parent_education = int(input("Parent Education Level (0: No college, 1: Some college, 2: Bachelor's, 3: Graduate): "))
    school_type = input("School Type (Public, Private, Charter): ")
    region = input("Geographic Region (Northeast, Southeast, Midwest, Southwest, West): ")
    essay_quality = float(input("Essay Quality (1 - 5): "))
    recommendation_strength = float(input("Recommendation Strength (1 - 5): "))

    # Encode categorical variables using the label encoders from the predictor
    school_type_enc = predictor.label_encoders['School_Type'].transform([school_type])[0]
    region_enc = predictor.label_encoders['Geographic_Region'].transform([region])[0]

    # Feature engineering (must match training)
    gpa_sat_interaction = gpa * sat_score
    academic_index = (gpa * 0.4 + sat_score/1600 * 0.6)
    activity_score = extracurricular + leadership + volunteer_hours/100
    socioeconomic_index = (family_income/100000 + parent_education/3) / 2

    # Order must match self.feature_names
    student_data = [
        gpa, sat_score, act_score, extracurricular, leadership, volunteer_hours,
        work_experience, family_income, parent_education, school_type_enc, region_enc,
        essay_quality, recommendation_strength, gpa_sat_interaction, academic_index,
        activity_score, socioeconomic_index
    ]
    # Scale numerical features
    student_df = pd.DataFrame([student_data], columns=predictor.feature_names)
    numerical_cols = student_df.select_dtypes(include=[np.number]).columns
    student_df[numerical_cols] = predictor.scaler.transform(student_df[numerical_cols])
    return student_df.iloc[0].values
...
def main():
    ...
    # --- Ask user for input and predict (moved up) ---
    student_data = get_user_input(predictor)
    prediction, probability = predictor.predict_admission(student_data)
    print(f"\nPrediction for the entered student: {'Admitted' if prediction == 1 else 'Rejected'} (Probability: {probability:.2f})")
